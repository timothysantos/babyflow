...
test('manual runtime smoke', async ({ page }) => {
  ...
  await page.getByTestId('quick-action-dock').getByRole('button', { name: 'Feed' }).click();
  const feed = await feedResponse;
  expect(feed.ok()).toBeTruthy();
  await expect(page.getByTestId('feed-session-item').first().getByTestId('feed-session-status')).toContainText('Live');
});
