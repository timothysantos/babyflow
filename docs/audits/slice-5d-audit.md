...
- Timeline, Journal, and Compact now expose reason capture and suggested-correction affordances, but the correction flow still lacks richer contextual guidance such as confidence scoring, duplicate warnings, and replay-oriented correction hints.
- The slice now proves delete/merge/restore/update parity across Timeline, Journal, Compact, and Correction History, but the correction surface is still intentionally narrow rather than fully assistive.
...
| `src/client/components/timeline/TimelineEditSheet.tsx` | WIRED | Timeline update sheet exposes reason capture and suggested-correction affordances. |
...
| Suggested correction and reason capture are visible. | VERIFIED | `tests/correction-assistive-ux.test.tsx` passed and proved reason selectors plus suggested-correction affordances on Timeline, Journal, and Compact edit surfaces. |
...
| Timeline edit now uses a first-class sheet instead of prompt | VERIFIED | `tests/correction-cross-surface-update.test.tsx` and `tests/correction-assistive-ux.test.tsx` passed with sheet-driven timeline editing and reason capture. |
...
- The remaining v8a work for this slice is richer correction UX: contextual suggestions, confidence scoring, duplicate warnings, and replay-oriented correction hints are still future work, even though the Timeline edit path is now first-class.
...
- Recommended next tests to implement after Slice 5D: add confidence-scored suggestions, duplicate-warning flows, and replay-oriented correction hints, then re-audit those flows under the same projection consistency rules.
- Slice commits:
