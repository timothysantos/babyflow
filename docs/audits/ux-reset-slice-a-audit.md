...
| Active feed supports elapsed timer, current-segment stopwatch, left/right/formula/note controls, manual time editing, and close feed from the operational screen. | VERIFIED | `tests/feed-active-timer.test.tsx` asserts elapsed timer, current segment switching, segment stopwatch, `Edit time`, `Save duration`, and `Close feed`. |
