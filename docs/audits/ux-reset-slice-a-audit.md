...
| Active feed supports elapsed timer, left/right/bottle/note controls, import duration, and close feed from the operational screen. | VERIFIED | `tests/feed-active-timer.test.tsx` asserts elapsed timer, segment buttons, `Import duration`, `Save duration`, and `Close feed`. |
