...
- [b513004 document today mobile ux repair audit commit](https://github.com/timothysantos/babyflow/commit/b5130042ab7dc83e3af51c710a7d9bd14f8dc12e)
...
| Top pill labels stay inside their pill bounds | VERIFIED | `tests/e2e/today-mobile.spec.ts` measures `Timeline / 时间线`, `Journal / 记录表`, and `Review / 复盘` at 390px and verifies each label has `scrollWidth <= clientWidth` with nowrap and ellipsis styling. |
...
| Active feed timing stays readable on mobile | VERIFIED | `tests/e2e/today-mobile.spec.ts` starts `Left feed` at 390px, measures `feed-session-status` and `active-feed-current-segment`, and verifies the timing/status chip and current segment remain inside the card and viewport. |
