# BabyFlow — Canonical Master Spec v8

**Status:** Canonical architecture spec after v2 merge + v3 critique audit + v5 UI/implementation expansion + v6 auth/caregiver access layer + v6b dynamic rolling recalculation engine + v7 benchmark/settings architecture + v8 paper-journal parity view correction  
...
# 12. Default UX — Paper Journal First, With Paper Journal View Parity

## 12.1 v8 Correction Summary

v7 correctly states that the app is a strong timeline journal powering decision support.

The missing correction is visual and operational:

```text
The app must include a mode that actually looks like the physical paper journal.
```

Without this, the implementation can drift into:

```text
cards + controls + event log
```

instead of:

```text
one journal page, one row per caregiving episode, with the same columns as the paper journal
```

v8 therefore adds a non-negotiable UI requirement:

```text
Today must support Timeline View, Paper Journal View, and Compact View.
```

The app can still be modern, mobile-first, and fast at 2am.

But the caregiver must be able to say:

```text
This app row is the same thing as the paper journal row.
```
...
```text
What happened in this episode, and which journal column should be filled?
```

Canonical v8 doctrine:

```text
The paper-journal row is the primary object.
The live timeline is the source of truth.
Quick actions are timeline stamps.
Details are drilldowns attached to the row.
...
```text
1. Timeline View
2. Paper Journal View
3. Compact View
```

These are projections of the same underlying data, not separate systems.
...
```text
Baby context
Current active cycle row summary
Timeline entries
Quick action dock
...
```text
- paper-to-app validation
- nanny/family handoff
- course/facilitator alignment
- end-of-day review
- bilingual export
- checking that the app did not lose the paper mental model
```

Shape:

```text
A table with the same columns and order as the paper journal.
```
...
- narrow screens
- tired caregivers
- users who dislike horizontal scroll
- quick row review
```
...
The Today page must include a view switcher.
...
## 12.5 Paper Journal View Is Required

Paper Journal View is not a nice-to-have.

It is required because:

```text
- the paper journal is the source mental model
...
## 12.7 Column-by-Column Behavior
...
```text
Start or reopen a recovery-regulation episode.
```

Scenario:

```text
Baby wakes crying at 6:30am.
Caregiver taps Wake.
The app creates or updates the active journal row.
```
...
```text
Start a feed session inside the active journal row.
```
...
```text
Capture feeding sequence, not only totals.
```
...
```text
RB20 · Burp5 · LB5 · Wake attempt · LB5
```
...
- Preserve segment chronology across repository, API, and UI.
...
```text
Baby is awake and engaged after feeding or settling.
```
...
```text
Put Down = caregiver begins sleep transition.
Asleep = baby actually fell asleep.
```
...
Paper Journal View shows Start of Sleep Time = 10:52.
...
```text
How long the sleep period lasted.
```
...
```text
Context, intuition, uncertainty, interruptions, and observations.
```
...
```text
- Paper Journal View matches paper column order.
...
## 12.9 Paper Journal View UI Specification
...
- Cells with missing/uncertain values show subtle review indicator.
```
...
## 12.10 PaperJournalRowViewModel
...
## 12.11 Quick Actions Must Be Timeline Stamps
...
## 12.12 Required Changes From Current Screenshot

### Remove or Move Marketing Hero

Current:

```text
BABYFLOW
Calm journaling for 2am caregiving
Baby profile
```

Required operational Today replacement:
...
```text
Noah · Week 3
Last feed 9:00 · Next likely feed 11:30–12:00
```

### Remove “Open Journal”

If the user is already on Today, the journal is already open.
...
```text
View mode: Timeline / Journal / Compact
```

### Reframe WAKE | FEED | PLAY | PUT DOWN | SLEEP

Current visual reads like tabs.
...
```text
Wake ✓ → Feed active → Play — → Put down — → Asleep —
```

### Rename “Show details”

Use row-attached language:
...
### Reframe “Condensed journal active”
...
or represent it inside the view mode switcher.

### Reframe Mark Buttons
...
Then show quick stamps.

### Reframe Note Buttons

Do not show disconnected note tiles as the main pattern.

Preferred:

```text
Note opens bottom sheet:
- General note
- Feeding note
- Sleep note
- Diaper note
...
Notes must attach to current row or selected timeline event.

---

## 12.13 v8 Target Mobile Layout
...
Add to timeline
[Wake] [Feed] [Burp] [Diaper]
[Play] [Put down] [Asleep] [More]
```

Paper Journal View mobile:
...
## 12.14 Paper Journal View Acceptance Criteria

Paper Journal View is complete only when:
...
[ ] Paper Journal View can be used to validate the paper form side-by-side.
```

---

## 12.15 Required Tests
...
- maps wake/feed/play/sleep/diaper/remarks values correctly
- preserves feed segment chronology inside Feed cell
- distinguishes put down from asleep
- derives sleep duration from sleep boundaries
...
## 12.16 v8 Today UI Definition of Done

The Today UI is not done unless:

```text
[ ] The current journal row is visibly the primary object.
[ ] Quick actions are framed as timeline stamps.
[ ] Paper Journal View exists and mirrors the physical journal.
[ ] Compact View exists for one-handed use.
[ ] Timeline View shows chronological event flow.
...
[ ] “Open journal” is removed from the Today page.
[ ] All row values are backed by domain events/sessions/segments/derived metrics.
```

---

## 12.17 v8 Corrective Slice

Add a corrective implementation slice before deeper interpretation work.

### Slice 5B — Paper Journal View Parity

```text
Goal:
Make the app visibly match the physical paper journal so caregivers can use
paper and app concurrently, and so implementation can be validated against the
journal column model before deeper interpretation work continues.
```
...
- PaperJournalRowViewModel
- ViewModeSwitcher
- Journal/Timeline/Compact view persistence
- column mapping tests
- mobile horizontal scroll proof
- bilingual header proof
- Scenario 1 row parity test
```

Files:

```text
src/client/components/journal/PaperJournalView.tsx
src/client/components/journal/PaperJournalTable.tsx
src/client/components/journal/PaperJournalRow.tsx
src/client/components/journal/ViewModeSwitcher.tsx
src/client/view-models/paper-journal-row-view-model.ts
tests/paper-journal-view.test.tsx
tests/paper-journal-view-model.test.ts
tests/e2e/paper-journal-mobile.spec.ts
```
...
```text
[ ] User can switch to Journal view.
[ ] Journal view resembles the physical paper journal.
[ ] Paper columns are correct and bilingual.
[ ] Same cycle values match between Timeline and Journal views.
[ ] Feed segment chronology appears correctly.
[ ] Sleep duration is derived.
[ ] Mobile horizontal scroll is verified at 390px.
[ ] View switching preserves data.
```

Do this before building more decision-support UI.

The app should not continue adding interpretation modules while the core journal surface still feels disconnected from the paper journal.
